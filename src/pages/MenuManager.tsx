...
const MOCK_LIBRARY: Dish[] = [
  { id: '1', name: 'Lemon Herb Grilled Chicken', type: 'Executive', kcal: 650, protein: 45, starred: true, status: 'Active' },
  { id: '2', name: 'Mediterranean Quinoa Bowl', type: 'Salad', kcal: 420, protein: 18, starred: false, status: 'Active' },
  { id: '3', name: 'Spicy Beef Stir Fry', type: 'Executive', kcal: 710, protein: 52, starred: false, status: 'Draft' }
];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', backgroundColor: '#1A1A1A', padding: '4px', borderRadius: '8px', border: '1px solid #333' }}>
          ...
          <TabButton active={activeTab === 'library'} onClick={() => setActiveTab('library')}>📚 Meal Library</TabButton>
        </div>
      </div>
                  ...
                  {dish ? (
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        ...
                        <div>
                          <h4 style={{ fontSize: '16px', fontWeight: 600, marginBottom: '4px' }}>{dish.name}</h4>
                          <span style={{ fontSize: '13px', color: '#9CA3AF' }}>{dish.type} · {dish.kcal} kcal</span>
                        </div>
                      </div>
            ...
            {MOCK_LIBRARY.map(dish => (
              <div key={dish.id} style={{ 
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '13px', color: '#9CA3AF' }}>{dish.type} · {dish.kcal} kcal · {dish.protein}g protein</div>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
                  ...
                  <button className="btn-ghost" style={{ padding: '8px' }}>✏️ Edit</button>
                </div>
              </div>
            ))}
          </div>

          <button className="btn-primary" style={{ alignSelf: 'flex-start', marginTop: '16px' }} onClick={() => navigate('/menu/add')}>
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 100 }}>
      <div style={{ backgroundColor: '#1A1A1A', width: '100%', maxWidth: '600px', borderRadius: '24px 24px 0 0', padding: '32px', position: 'relative', borderTop: '1px solid #333', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
          ...
          {filtered.map((dish: Dish) => {
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <h4 style={{ fontWeight: 600, fontSize: '15px' }}>{dish.name}</h4>
                  <div style={{ fontSize: '13px', color: '#9CA3AF' }}>{dish.type} · {dish.kcal} kcal</div>
                </div>
        ...
        <div style={{ display: 'flex', gap: '16px', marginTop: 'auto' }}>
          ...
          <button className="btn-primary" style={{ flex: 2 }} disabled={!selected} onClick={() => selected && onAssign(selected)}>
