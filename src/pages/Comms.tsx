...
export default function Comms() {
  ...
  const [message, setMessage] = useState('Hope you enjoyed your Zaadi lunch! 🍛 Share your thoughts with us to get 10% off your next renewal.');

  const getRecipientCount = () => {
    switch(segment) {
      case 'All active': return 248;
      case 'Month plan': return 129;
      case 'Lapsed': return 42;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      {activeTab === 'automations' ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <h3 style={{ fontSize: '14px', color: '#9CA3AF', letterSpacing: '1px', margin: 0 }}>ACTIVE AUTOMATIONS</h3>
            ...
            {AUTOMATIONS.map(auto => (
              <div key={auto.id} style={{ 
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                  <div style={{ 
                    width: '48px', height: '48px', borderRadius: '12px', backgroundColor: '#222', 
                  ...
                  <div>
                    <h4 style={{ fontSize: '18px', fontWeight: 600, marginBottom: '6px', color: toggles[auto.id] ? '#FFF' : '#9CA3AF' }}>{auto.name}</h4>
                    <p style={{ fontSize: '14px', color: '#9CA3AF', margin: 0 }}>{auto.desc}</p>
                  </div>
                </div>

                <ToggleSwitch isOn={toggles[auto.id]} onToggle={() => setToggles({ ...toggles, [auto.id]: !toggles[auto.id] })} />
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', maxWidth: '800px' }}>
          <h3 style={{ fontSize: '14px', color: '#9CA3AF', letterSpacing: '1px', margin: 0 }}>SEND BROADCAST</h3>
          ...
          <div style={{ backgroundColor: '#1A1A1A', padding: '32px', borderRadius: '16px', border: '1px solid #333', display: 'flex', flexDirection: 'column', gap: '32px' }}>
            ...
            {/* Segment Selector */}
            <div>
              <label style={{ display: 'block', fontSize: '14px', color: '#D1D5DB', fontWeight: 600, marginBottom: '12px' }}>Audience Segment</label>
  ...
  return (
    ...
    >
      <div style={{
        ...
        position: 'absolute', top: '2px', left: isOn ? '20px' : '2px',
