...
export default function RevenueDashboard() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <div style={{ backgroundColor: '#1A1A1A', padding: '32px', borderRadius: '16px', border: '1px solid #333', position: 'relative' }}>
        ...
        <div style={{ color: '#9CA3AF', fontSize: '13px', fontWeight: 600, letterSpacing: '1px', marginBottom: '8px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
        ...
        <div style={{ backgroundColor: '#1A1A1A', padding: '24px', borderRadius: '16px', border: '1px solid #333' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
            <h3 style={{ fontSize: '14px', color: '#9CA3AF', letterSpacing: '1px', margin: 0 }}>DAILY REVENUE</h3>
            <select style={{ backgroundColor: '#222', color: '#FFF', border: '1px solid #333', padding: '4px 8px', borderRadius: '4px', fontSize: '13px' }}>
        ...
        <div style={{ backgroundColor: '#1A1A1A', padding: '24px', borderRadius: '16px', border: '1px solid #333' }}>
          <h3 style={{ fontSize: '14px', color: '#9CA3AF', letterSpacing: '1px', margin: 0, marginBottom: '24px' }}>
            SUBSCRIBERS BY PLAN
          </h3>
...
function StatTile({ label, value, color = '#FFF', onClick }: any) {
  return (
    <div onClick={onClick} style={{ 
      backgroundColor: '#222', padding: '16px 20px', borderRadius: '12px', 
...
function BarChartCol({ day, height, opacity, today }: any) {
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', height: '100%' }}>
      ...
      <div style={{ 
        fontSize: '12px', color: today ? '#FFF' : '#9CA3AF', 
...
function HorizontalBar({ label, count, width, color }: any) {
  const navigate = useNavigate();
  return (
    <div onClick={() => navigate('/customers?plan=' + label.toLowerCase())} style={{ cursor: 'pointer' }}>
