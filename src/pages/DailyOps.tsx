...
const MOCK_ISSUES: Issue[] = [
  { id: '1', type: '😟 Wrong order', customer: 'Ahmad Alsaud', location: 'Olaya Towers, Floor 14', text: 'I received a Salad meal instead of my Executive meal.' },
  { id: '2', type: '⏰ Late delivery', customer: 'Sara Aljohani', location: 'KAFD Area 4, Desk 2B', text: 'It is 1:30 PM and my lunch hasn\'t arrived yet.' }
];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <h3 style={{ fontSize: '14px', color: '#9CA3AF', letterSpacing: '1px' }}>MEAL BREAKDOWN</h3>
            {role === 'Admin' && (
              <div style={{ display: 'flex', gap: '8px' }}>
                <button className="btn-ghost" style={{ padding: '8px 16px', border: '1px solid #333' }}>📄 Export</button>
                <button className="btn-primary" style={{ padding: '8px 16px' }} onClick={() => navigate('/labels')}>🏷️ Labels</button>
              </div>
            )}
          </div>
          <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', border: '1px solid #333', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '16px', backgroundColor: '#222' }}>
        ...
        {/* 3c. OPEN ISSUES */}
        <div>
          <h3 style={{ fontSize: '14px', color: '#9CA3AF', letterSpacing: '1px', marginBottom: '16px' }}>
            OPEN ISSUES ({issues.length})
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {issues.length === 0 ? (
              <div style={{ padding: '24px', textAlign: 'center', color: '#9CA3AF', border: '1px dashed #333', borderRadius: '12px' }}>
              ...
              <div key={issue.id} style={{ backgroundColor: '#1A1A1A', padding: '20px', borderRadius: '12px', border: '1px solid #333' }}>
                ...
                <p style={{ color: '#9CA3AF', fontSize: '14px', marginBottom: '12px' }}>{issue.location}</p>
                <p style={{ fontSize: '15px', fontStyle: 'italic', marginBottom: '20px', backgroundColor: '#222', padding: '12px', borderRadius: '8px' }}>
                  "{issue.text}"
                </p>
      ...
      {activeIssueForCredit && (
        <CreditModal 
          ...
          onClose={() => { setActiveIssueForCredit(null); setCreditAmount(''); }} 
      ...
      {activeIssueForReject && (
        <RejectModal 
          ...
          onClose={() => { setActiveIssueForReject(null); setRejectReason(''); }} 
...
function StatusStep({ label, status, icon }: { label: string, status: 'completed' | 'active' | 'pending', icon: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', zIndex: 1 }}>
      ...
      <span style={{ fontSize: '14px', fontWeight: isActive ? 600 : 400, color: status === 'pending' ? '#6B7280' : '#FFF' }}>{label}</span>
    </div>
  );
}
    ...
    <ModalOverlay onClose={onClose}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '24px' }}>
        <div style={{ position: 'relative', width: '200px' }}>
          <span style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)', color: '#9CA3AF', fontSize: '20px' }}>SAR</span>
          <input 
            type="number" 
            value={amount}
      ...
      <div style={{ textAlign: 'center', color: '#9CA3AF', fontSize: '13px', marginBottom: '12px' }}>Maximum credit: SAR 30 per issue</div>
      <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', marginBottom: '32px' }}>
        {['10', '20', '28', '30'].map(preset => (
          <button key={preset} onClick={() => setAmount(preset)} style={{ backgroundColor: '#222', color: '#FFF', padding: '8px 16px', borderRadius: '20px', border: '1px solid #444' }}>SAR {preset}</button>
        ))}
      </div>

      <div style={{ display: 'flex', gap: '16px' }}>
        <button className="btn-ghost" style={{ flex: 1, backgroundColor: '#222' }} onClick={onClose}>Cancel</button>
        <button className="btn-primary" style={{ flex: 2 }} disabled={isError || !amount} onClick={onConfirm}>
...
function ModalOverlay({ children }: any) {
  return (
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 100 }}>
      <div style={{ backgroundColor: '#1A1A1A', width: '100%', maxWidth: '500px', borderRadius: '24px 24px 0 0', padding: '32px', position: 'relative', borderTop: '1px solid #333' }}>
